...
PlotCallout::PlotCallout(QChart *chart):
        QGraphicsItem(chart),
        m_chart(chart)
{
}

QRectF PlotCallout::boundingRect() const
{
...
void PlotCallout::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    Q_UNUSED(option)
    Q_UNUSED(widget)
    QPainterPath path;
...
void PlotCallout::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
    event->setAccepted(true);
}

void PlotCallout::mouseMoveEvent(QGraphicsSceneMouseEvent *event)
{
...
void PlotCallout::setText(const QString &text)
{
    m_text = text;
...
void PlotCallout::setAnchor(QPointF point)
{
    m_anchor = point;
}

void PlotCallout::updateGeometry()
{
    prepareGeometryChange();
